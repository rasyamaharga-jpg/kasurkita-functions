...
export default async function handler(req, res) {
  ...
  return { statusCode: 502, headers, body: JSON.stringify({ status: 'error', message: 'Layanan pengiriman sedang gangguan.', debug: errText }) };
}
    const data = await response.json();
    if (!data.success) {
  console.error('[cek-ongkir] Biteship error:', JSON.stringify(data));
  return { statusCode: 400, headers, body: JSON.stringify({ status: 'error', message: 'Tidak ada kurir tersedia', debug: data }) };
}
