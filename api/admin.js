// api/admin.js - Lengkap dengan semua action termasuk stok per motif
...
export default async function handler(req, res) {
  ...
  if (!body || typeof body !== 'object') {
    try { body = JSON.parse(req.body); } catch (e) {
      return res.status(400).json({ error: 'Body tidak valid JSON' });
    }
  }
  ...
  try {
    ...
    if (action === 'updateStokPerMotif') {
      ...
      if (!idVarian || !idMotif || stokBaru === undefined) {
        return res.status(400).json({ error: 'idVarian, idMotif, stokBaru diperlukan' });
      }
    ...
    if (action === 'updateStatus') {
      ...
      if (!valid.includes(statusBaru)) return res.status(400).json({ error: 'Status tidak valid' });
