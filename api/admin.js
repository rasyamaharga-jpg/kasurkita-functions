...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (action === 'getAllPesanan') {
      const response = await fetch(`${SUPABASE_URL}/rest/v1/pesanan?select=*&order=tanggal.desc`, {
    ...
    if (action === 'getProdukAdmin') {
      const response = await fetch(`${SUPABASE_URL}/rest/v1/produk?select=*&order=lebar.asc,tinggi_kasur.asc`, {
        headers: { apikey: SUPABASE_KEY, Authorization: `Bearer ${SUPABASE_KEY}` }
      });
      const data = await response.json();
