// api/buat-resi.js - Buat resi via Biteship
export default async function handler(req, res) {
  ...
  if (!adminPass || adminPass !== process.env.ADMIN_PASS) { await new Promise(r => setTimeout(r,1000)); return res.status(403).json({ status: 'error', message: 'Akses ditolak' }); }
  if (!pesanan || !courierCode || !courierService || !destinationAreaId) return res.status(400).json({ status: 'error', message: 'Data tidak lengkap' });
  ...
  try {
    const response = await fetch('https://api.biteship.com/v1/orders', {
      ...
      body: JSON.stringify({
        ...
        destination_contact_name: String(pesanan.nama).slice(0,100), destination_contact_phone: String(pesanan.noWA).replace(/\D/g,'').slice(0,15), destination_address: String(pesanan.alamat).slice(0,255), destination_area_id: destinationAreaId, destination_postal_code: 0,
